...
class ch11_MonadTests extends FunSuite {
  ...
  test("11.6 - filterM") {
    ...
    assert(None == optionMonad.filterM(List(1, 2, 3, 4, 5)) { (i: Int) =>
      if (i == 4) None else Some(true)
    })
  ...
  test("11.18 - state monad: sequence") {
    ...
    def doCount(s: String): myState[String] = State { counter =>
      (s + " executed!", counter + 1)
    }

    val cmds = List("cd", "ls", "pwd")
    val states: List[myState[String]] = cmds.map { cmd =>
      doCount(cmd)
    }
    ...
    assert(3 == totalCounted_state,
           "sequence of accumulated single incremental counts should result in the total count")
  }
  ...
  test("11.20 - state laws") {
    ...
    assert(1 == r2.run(0)._2,
           "law: setting the state, should return the last set state when getting the state")

    val r3 = for {
      x <- F.unit("abc")
    } yield x
    assert("abc" == r3.run(2)._1,
           "law: unit value should be returned when 'directly' running the state monad.")
  ...
  test("11.21 - Reader monad") {
    ...
    assert(
      List(12, 12, 12) == replicated.run(2),
      "replicating of the reader monad: apply function n times with same arg and return the result")
    ...
    assert(
      List(10, 2) == sequenced.run(4),
      "sequence turn a list of functions in to a function that takes one argument and returns the results in a list")
